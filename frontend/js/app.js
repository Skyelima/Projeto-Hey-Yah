...
function initLogin() {
  document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.status === 401 && !loadData('loggedIn', false)) {
      ...
      if (res.ok) {
        ...
        saveData('userEmail', data.user.email);
        saveData('userName', data.user.name);
...
async function syncFromBackend() {
  ...
  try {
    // Busca tarefas
    const resT = await fetch('http://localhost:3000/api/tasks', {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (resT.ok) {
      const serverTasks = await resT.json();
      saveTasks(serverTasks); // Sobrescreve local
    }
    
    // Busca gamificação
    const resG = await fetch('http://localhost:3000/api/gamification', {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (resG.ok) {
      const gamif = await resG.json();
      const user = getUserData();
      user.pontos = gamif.xp || 0;
      user.nivel = (gamif.estagioAtual || 0) + 1;
      saveUserData(user);
    }
  } catch(e) {
    console.error('Offline mode - could not sync', e);
  }
}
...
function toggleTaskStatus(id) {
  ...
  saveTasks(tasks);
  pushTask({ status: task.status }, 'PUT', id);
  ...
  if (!wasCompleted) {
    ...
    // Sincroniza XP no backend
    const token = loadData('token', '');
    if (token) {
      fetch('http://localhost:3000/api/gamification/xp', {
        method: 'PUT',
...
async function pushTask(taskData, method = 'POST', id = '') {
  const token = loadData('token', '');
  if (!token) return;
  try {
...
document.getElementById('taskForm').addEventListener('submit', (e) => {
  ...
  if (editId) {
    ...
    pushTask(taskData, 'PUT', editId);
  } else {
    // UC3 - Create
    const newId = tasks.length ? Math.max(...tasks.map(t=>t.id)) + 1 : 1;
    const newTask = { id: newId, ...taskData, status: 'pendente', criadoEm: Date.now() };
    tasks.push(newTask);
    const userData = getUserData();
    userData.totalCreated = (userData.totalCreated||0) + 1;
    saveUserData(userData);
    pushTask(newTask, 'POST');
  }
...
async function generateAISuggestions() {
  ...
  btn.textContent = '⏳ Processando com a Nuvem...';
  ...
  try {
    ...
    document.getElementById('aiSuggestionsList').innerHTML = '<div class="glass-card ai-card"><h4>Erro</h4><p>Não foi possível comunicar com o backend da Inteligência Artificial. Tente novamente mais tarde.</p></div>';
  } finally {
